import cv2
from matplotlib import pyplot as plt
import os
import numpy as np
from tensorflow.keras.preprocessing.image import load_img
from tensorflow.keras.preprocessing.image import img_to_array
from tensorflow.keras.models import load_model

filepath = 'E:/jeeva project/Mini1/Plant-Leaf-Disease-Prediction-main/model.h5'
model = load_model(filepath)
print(model)

print("Model Loaded Successfully")

image_path = r'E:/jeeva project/Mini1/Plant-Leaf-Disease-Prediction-main/Dataset/test/Guava__Bacteria(1).jpg'
tomato_plant = cv2.imread(image_path)
if tomato_plant is None:
    print(f"Error: Unable to load image from {image_path}")
else:
    # Resize the image if it is loaded correctly
    test_image = cv2.resize(tomato_plant, (128, 128))
    print("Image loaded and resized successfully.")

test_image = cv2.resize(tomato_plant, (128,128)) # load image 
  
test_image = img_to_array(test_image)/255 # convert image to np array and normalize
test_image = np.expand_dims(test_image, axis = 0) # change dimention 3D to 4D
  
result = model.predict(test_image) # predict diseased palnt or not
  
pred = np.argmax(result, axis=1)
print(pred)
if pred==1:
    print("Guava - Bacterial Affect")

elif pred==2:
    print("Papaya - Viral Affect")
    
elif pred==3:
    print( "Tomato - Bacteria Spot Disease")
       
elif pred==4:
    print("Tomato - Early Blight Disease")
    
        
elif pred==5:
    print("Tomato - Healthy and Fresh")
        
elif pred==6:
    print("Tomato - Late Blight Disease")
       
elif pred==7:
    print("Tomato - Leaf Mold Disease")
        
elif pred==8:
    print("Tomato - Septoria Leaf Spot Disease")
        
elif pred==9:
    print("Tomato - Target Spot Disease")
        
elif pred==10:
      print("Tomato - Tomoato Yellow Leaf Curl Virus Disease")
elif pred==11:
      print("Tomato - Tomato Mosaic Virus Disease")
        
elif pred==12:
      print("Tomato - Two Spotted Spider Mite Disease")
